""" 
File for testing trained LeNet (using Reinhard Stain Normalization) on images by iterating over every pixel
"""
...
model.load_state_dict(torch.load('reinhard_dump/thin_film_LeNet_dict.pt'))
...
file_list = os.listdir('reinhard_tool/')

# file_list = file_list[:100]

start = time.time()

done_list = os.listdir('reinhard_pkl_dump/')
...
for file_name in file_list:
    ...
    img = cv2.imread('reinhard_tool/' + file_name)
    orig_img = img
...
#    weighted = cv2.addWeighted(orig_img, 0.7, overlay, 0.3, 0, dtype=cv2.CV_64F)
    ...
    # cv2.imwrite('reinhard_dump/'+file_name + '_overlay.png', weighted)
...
#    weighted = cv2.addWeighted(orig_img, 0.7, overlay, 0.3, 0, dtype=cv2.CV_64F)
    ...
    # cv2.imwrite('reinhard_dump/'+file_name + '_overlay_area_filter.png', weighted)

    
    f = open('reinhard_pkl_dump/'+file_name + '_classmap.pkl', 'wb')
    pickle.dump(class_map, f)
    f.close()

    f = open('reinhard_pkl_dump/'+file_name + '_heatmap.pkl', 'wb')
