""" 
File for testing trained LeNet (using Macenko Stain Normalization) on images by iterating over every pixel
"""
...
model.load_state_dict(torch.load('thin_film_LeNet_dict.pt'))
...
file_list = os.listdir('macenko')

# file_list = file_list[:100]

start = time.time()

done_list = os.listdir('macenko_pkl_dump/')
...
#	Check if files already present in the folder
    ...
    img = cv2.imread('macenko/' + file_name)
    if img is None:
        continue
...
#heatmap_img = cv2.applyColorMap(overlay, cv2.COLORMAP_JET)
    ...
    #cv2.imwrite('macenko_dump/'+file_name + '_overlay.png', weighted)
...
# # Overlay the image with the RGB Prop matrix
    ...
    #cv2.imwrite('macenko_dump/'+file_name + '_overlay_area_filter.png', weighted)

    
    f = open('macenko_pkl_dump/'+file_name + '_classmap.pkl', 'wb')
    pickle.dump(class_map, f)
    f.close()

    f = open('macenko_pkl_dump/'+file_name + '_heatmap.pkl', 'wb')
